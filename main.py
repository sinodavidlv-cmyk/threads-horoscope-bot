...
3. 強力引導互動：結尾必須包含引導 Tag 朋友或留言自首的行動呼籲（CTA）。

【貼文要求】
1. 開頭標題：格式必須為 "{today_str}今日12星座最佳配對: "，後面接一句超吸睛的犀利金句。
2. 配對內容：精簡列出今日「愛情」、「友情」、「職場」的最佳配對組合，並附上各一組「互動火花/吐槽短評」。
3. 討喜排版：使用簡短文字與 Emoji，適合手機快速瀏覽。
4. 結尾：加入「@標記你的星座朋友/隊友」或「留言你的星座」的互動問句。
5. 字數控制：全部字數（含符號與 Emoji）請嚴格控制在 420 字以內，留出閱讀呼吸感。
"""
